...
| `PACKAGE_NAME` | Variable | 无 | 要同步的唯一 npm 包名，例如 `@scope/pkg`。 |
...
1. 执行 `npm pack <pkg>@<version> --registry <source>` 从源 registry 拉取 tarball。
2. 解压 tarball。
...
5. 不修改 `version`。
6. 不修改业务文件。
7. 重新打包后执行 `npm publish <tgz> --registry <target> --tag <tag> --access <TARGET_ACCESS>`，默认 `public`。
