...
| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `SOURCE_REGISTRY_URL` | Secret | 无 | 私有 npm-compatible registry URL。必须用 Secret，避免 GitHub Actions step header 暴露原 registry 地址。 |
| `SOURCE_REGISTRY_TOKEN` | Secret | 无 | 读取源 registry 和 `npm pack` 使用的 token。 |
| `PACKAGE_NAME` | Secret | 无 | 源 registry 上的 npm 包名，例如 GitLab 中的 `@internal/pkg`。必须用 Secret，避免 GitHub Actions step header 暴露源包名。 |
...
`SOURCE_REGISTRY_URL` 和 `PACKAGE_NAME` 必须配置为 GitHub Secrets，不要配置为 Variables。GitHub runner 会在每个 step 开始前打印 env；Variables 在首次打印前无法可靠隐藏，Secrets 会由 GitHub 自动 mask。

脚本也会在 GitHub Actions 中注册 mask，隐藏：
